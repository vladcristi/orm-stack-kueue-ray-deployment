...
variable "operator_kubectl_files_path" {
  type        = string
  default     = ""
  description = "The directory on the operator host where to push the files which will be run with kubectl apply -f."
}

variable "kubectl_yaml_path" {
  type        = string
  description = "The local path to the yaml files."
}
